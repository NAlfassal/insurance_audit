...
**Evidence-Based Confidence:** Confidence scores reflect empirical
accuracy — **0.97** for structural breaches, **0.93** for contract pricing
mismatches, and **0.97** for clean, fully re-priced invoices (slightly below the
dev set's 1.000 to prevent overfitting). Rows with daily cap violations receive
...
- **A rule read from one contract into another.** hospitals 1, 2 and 4 state
...
3. **Execute LLM Cross-Check:** Run the secondary contract-parser LLM to catch
   edge cases where regex patterns fail independently.
