...
Five choices the text leaves open were decided by the hospital_1 labels: an
exclusion window includes day N; a repeat of the same service, patient and date
is not payable; an out-of-term date is flagged but not zeroed; an unknown
service keeps its billed amount; a reused invoice id is reported with the later
record's totals. One of them is also read from one contract into another:
hospitals 1, 2 and 4 say an exclusion window is "measured in either direction"
from the service date, while hospitals 3 and 5 say nothing about direction, so
I applied the same rule there. *Example:* if hospital_3's windows ran backwards
only, its 3 excluded lines would need re-checking.
...
- **Pricing only fully matched invoices** — one unknown line hid 16 of the 25
  errors I missed.
